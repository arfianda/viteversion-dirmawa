...
import { CheckCircle, XCircle, Eye, Clock, AlertCircle, RefreshCw, Download, FileText, Search } from 'lucide-react';
...
export default function ScholarshipApplicationsQueue() {
  ...
  return (
    <div className="space-y-6 animate-fade-in pb-10">
      ...
      {modal.type && modal.application && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm overflow-y-auto">
          <div className="bg-white border border-slate-200 w-full max-w-md rounded-3xl shadow-2xl flex flex-col">
              ...
              {modal.type === 'detail' && (
                <div className="space-y-3 font-sans">
                  ...
                  {modal.application.document_url && (
                    <div className="border-t border-slate-100 pt-3">
                      <span className="text-slate-400 font-semibold block mb-2">Dokumen CV &amp; Transkrip</span>
                      <a
                        href={modal.application.document_url}
                        target="_blank"
                        rel="noreferrer"
                        className="inline-flex items-center gap-2 px-4 py-2 bg-slate-50 hover:bg-slate-100 text-[#001e40] font-bold rounded-xl border border-slate-200 transition-all cursor-pointer"
                      >
                        <FileText size={14} />
                        <span>Lihat Dokumen Pendaftaran</span>
                      </a>
                    </div>
                  )}
            ...
            {/* Modal Footer */}
            <div className="bg-slate-50 border-t border-slate-100 px-6 py-4 flex justify-end gap-2 rounded-b-3xl">
              <button
                onClick={() => setModal({ type: null, application: null })}
