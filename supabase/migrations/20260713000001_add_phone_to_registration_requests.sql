-- Migration: Add phone to registration requests and update approval RPC
-- Description: Add phone column to registration_requests and ensure it propagates to users table on approval.

-- 1. Add phone column to registration_requests
ALTER TABLE public.registration_requests ADD COLUMN IF NOT EXISTS phone TEXT;

-- 2. Update approve_registration_request RPC to copy phone
...
BEGIN
  ...
  INSERT INTO public.users (id, email, name, role, is_approved, phone)
  VALUES (v_user_id, v_req.email, v_req.name, 'mahasiswa', true, v_req.phone)
  ON CONFLICT (id) DO UPDATE
  SET role = 'mahasiswa', is_approved = true, phone = EXCLUDED.phone;
